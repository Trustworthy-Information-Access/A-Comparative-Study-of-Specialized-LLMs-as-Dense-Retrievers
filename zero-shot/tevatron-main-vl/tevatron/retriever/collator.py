...
def format_query(query: str, tokenizer=None) -> str:
    messages = [{'role': 'user',
            'content': f'Query: "{query}". Use one word to represent the query in a retrieval task. Make sure your word is in lowercase.'},
            {'role': 'assistant',
            'content': 'The word is: "'}]
    utility_prompt = tokenizer.apply_chat_template(messages, tokenize=False, add_generation_prompt=False)
    utility_prompt = utility_prompt.split('The word is: "')[0] + 'The word is: "'
    # print(utility_prompt)
    return utility_prompt

def format_passage(text: str, tokenizer=None) -> str:
    messages = [{'role': 'user',
            'content': f'Passage: "{text}". Use one word to represent the passage in a retrieval task. Make sure your word is in lowercase.'}, {'role': 'assistant',
            'content': 'The word is: "'}]
    utility_prompt = tokenizer.apply_chat_template(messages, tokenize=False, add_generation_prompt=False)
    utility_prompt = utility_prompt.split('The word is: "')[0] + 'The word is: "'
    # print(utility_prompt)
...
class MultiModalEncodeCollator:
    ...
    def __call__(self, features):
        ...
        for idx in range(len(texts)):
            ...
            # message = [{'role': 'user',
            # 'content': f'Query: "{text}". Use one word to represent the query in a retrieval task. Make sure your word is in lowercase.'},
            # {'role': 'assistant',
            # 'content': 'The word is: "'}]
            # messages.append(message)
            if self.data_args.encode_is_query:
                all_texts.append(format_query(text, tokenizer=self.processor))
            else:
                all_texts.append(format_passage(text, tokenizer=self.processor))
