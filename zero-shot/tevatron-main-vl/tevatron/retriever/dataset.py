...
class EncodeDataset(Dataset):
    ...
    def __getitem__(self, item):
        ...
        if self.data_args.encode_is_query:
            ...
            elif 'text' in content: # integrate with tevatron-v1 format
                content_text = content['text']
            else:
                assert 1 > 2
            # content_text = self.data_args.query_prefix + content_text
            # content_text = format_query(content_text, tokenizer = self.tokenizer)

            content_image = content['query_image'] if 'query_image' in content else None
        else:
            # content_text = format_passage(content['text'], content['title'], tokenizer = self.tokenizer)
            content_id = content['_id']
